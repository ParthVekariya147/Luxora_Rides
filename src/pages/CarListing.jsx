...
const CarListing = () => {
        ...
        const fetchCars = async () => {
            try {
                ...
                if (Array.isArray(response)) {
                    ...
                    } else if (response.success && response.data && Array.isArray(response.data.cars)) {
                        carsData = response.data.cars;
                    } else {
    ...
    const handleSortChange = (e) => {
        ...
        switch (sortValue) {
            ...
            case 'rating':
                // Rating: High to Low
                sortedCars.sort((a, b) => (b.ratings || b.average_rating || 0) - (a.ratings || a.average_rating || 0));
                break;
            case 'name':
                // Car Name: A to Z
                ...
                <Row>
...

                    {
                        Array.isArray(cars) && cars.length > 0 ? (
                            cars.map(item => <CarItem item={item} key={item.id || item._id} />)
                        ) : (
                            <Col lg='12'>
                                <div className="text-center mt-5">
